...
// The project page.
//
// Swapped IN PLACE on 2 Sep 2026 — same URL, new layout. That matters more than
// it sounds: roughly 120 revalidatePath('/brands/../projects/..') calls across
// actions.ts and results-actions.ts point here, and notifications.link is a
// PERSISTED column holding this URL on rows already in the table. Moving the
// route would have broken every one of them, including links in notifications
// sent weeks ago. Swapping what the route renders breaks none.
//
// The previous single-scroll implementation is in git history; reverting is one
// `git revert` of the cutover commit. The old /preview/project/[projectId] route
// still works and renders the same view, so a bookmark keeps working.
